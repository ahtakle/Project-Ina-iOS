...
class BabyProgress: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        //Sets up for remembering checked
        //Daily Checklist
        ...
        if (preferences.object(forKey: "Trim1Checklist") == nil) {
            arrayTrim1 = [Bool](repeating: false, count: trimester1Checklist.count)
        } else {
            arrayTrim1 = preferences.object(forKey: "Trim1Checklist") as? [Bool] ?? [Bool]()
        }
        //Trimester 2 Checklist
        if (preferences.object(forKey: "Trim2Checklist") == nil) {
            arrayTrim2 = [Bool](repeating: false, count: trimester2Checklist.count)
        } else {
            arrayTrim2 = preferences.object(forKey: "Trim2Checklist") as? [Bool] ?? [Bool]()
        }
        //Trimester 3 Checklist
        if (preferences.object(forKey: "Trim3Checklist") == nil) {
            arrayTrim3 = [Bool](repeating: false, count: trimester3Checklist.count)
        } else {
            arrayTrim3 = preferences.object(forKey: "Trim3Checklist") as? [Bool] ?? [Bool]()
        }
        ...
        if preferences.object(forKey: birthdayKey) as! String? == "" {
            ...
            if preferences.object(forKey: dueDateKey) as! String? != "" {
                ...
                let progressInt = Int(percentCompleted*100)
                progressLabel.text = "Progress: " + String(describing: progressInt) + "%"
                progressBar.setProgress(percentCompleted, animated: false)
                //Thickens Progress Bar (UI)
                ...
                let contentSize = informationTextView.sizeThatFits(informationTextView.bounds.size)
                var frame = informationTextView.frame
                frame.size.height = contentSize.height
                informationTextView.frame = frame
            }
            //LOGIC FOR IF NEITHER DUE DATE OR BDAY
            else {
                informationTextView.text = "Congratulations on you/your loved ones adventure into motherhood, and thank you for downloading this app! To access the information for this feature, please enter a due date or birthday for your baby."
                informationTextView.textColor = UIColor.white
                informationTextView.backgroundColor = UIColor.clear
                self.view.backgroundColor = UIColor.purple
                self.progressLabel.isHidden = true
                self.progressBar.isHidden = true
                self.babyImage.isHidden = true
                self.dday_bday_Label.isHidden = true
                self.week_month_Label.isHidden = true
                self.checklistControl.isHidden = true
        ...
        // LOGIC FOR BIRTHDAY
            ...
            //Hide Progress Bar and Checklist
            self.progressBar.isHidden = true
            self.checklistControl.isHidden = true
            self.tableView.isHidden = true
            ...
            dday_bday_Label.text = "Birth Date: " + dateString
            week_month_Label.text = String(monthsCompleted) + " Months Old"
            progressLabel.text = "Month " + String(monthsCompleted) + " Facts"
            ...
            //Change height because more information for monthly
            let contentSize = informationTextView.sizeThatFits(informationTextView.bounds.size)
            var frame = informationTextView.frame
            frame.size.height = contentSize.height
            informationTextView.frame = frame
    ...
    override func viewWillDisappear(_ animated: Bool) {
        ...
        //Save Checklist
        preferences.set(arrayDaily, forKey: "DailyChecklist")
        preferences.set(arrayTrim1, forKey: "Trim1Checklist")
        preferences.set(arrayTrim2, forKey: "Trim2Checklist")
        preferences.set(arrayTrim3, forKey: "Trim3Checklist")
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case 0:
            ...
            //Checks the item if already checked
            if (arrayDaily[indexPath.row] == true){
        ...
        case 1:
            ...
            if (arrayTrim1[indexPath.row] == true){
                self.tableView.selectRow(at: indexPath, animated: false, scrollPosition: UITableViewScrollPosition.top)
            }
        case 2:
            row = trimester2Checklist[indexPath.row]
            if (arrayTrim2[indexPath.row] == true){
                self.tableView.selectRow(at: indexPath, animated: false, scrollPosition: UITableViewScrollPosition.top)
            }
        case 3:
            row = trimester3Checklist[indexPath.row]
            if (arrayTrim3[indexPath.row] == true){
                self.tableView.selectRow(at: indexPath, animated: false, scrollPosition: UITableViewScrollPosition.top)
            }
        default:
            row = dailyChecklist[indexPath.row]
    ...
    func tableView(_ tableView: UITableView, didDeselectRowAt indexPath: IndexPath) {
        // cell deselected code here
        switch checklistControl.selectedSegmentIndex {
        case 0:
            arrayDaily[indexPath.row] = false
        case 1:
            arrayTrim1[indexPath.row] = false
        case 2:
            arrayTrim2[indexPath.row] = false
        case 3:
            arrayTrim3[indexPath.row] = false
        default:
            arrayDaily[indexPath.row] = false
        }
    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        case 1:
            arrayTrim1[indexPath.row] = true
        case 2:
            arrayTrim2[indexPath.row] = true
        case 3:
            arrayTrim3[indexPath.row] = true
        default:
            arrayDaily[indexPath.row] = true
        }
    }
